...
# import dataset
ID = 519
uciDataset = fetch_ucirepo(id=ID)
# Feature Engineering
...
print(X_predictData)
midData = pd.DataFrame([X_test.mean()]).round(0)
...
# 全包含的SHAP值(精準SHAP值)
ANS_LIST = [-0.0021577910573356007, 0.0023783895703752833, -0.010878907055409837, -6.441188044448154e-05, -0.061201777969585036, 
            4.306066198006553e-05, 0.00476648909501054, -0.11071752004308544, 0.0043982091635492715, 0.007234356439765666, 
            0.00030411650101624943, -0.05281863806763365]
LOCATION = f"SHAPSampling\\plot_data\\{ID}\\"
ROUND = 100 # 要計算幾次
MODE = 2 # 隨機方法0, 傳統費氏(凹型)1, 黃金抽樣2, 平均費氏3, 對稱費氏(凸型)4, 分層費氏5
GAP_LIMIT = 0.3788626020191722 # 保存上限設定值(mode4)
SAMPLING_NUM = 34 # 隨機選取特徵子集的數量34(mode4)
...
for j in range(ROUND):
    ...
    if MODE == 1:
        break
if not MODE == 1:
    print(f"此為ID{519}資料集, mode{MODE}, 總做了{ROUND}次")
