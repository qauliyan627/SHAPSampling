...
MODE = 4 # 隨機方法0, 傳統費氏(凹型)1, 黃金抽樣(低序列差異)2, 平均費氏3, 對稱費氏(凸型)4, 分層費氏5
COMP_MODE = 4
# 隨機選取特徵子集的數量(mode4)
SAMPLING_NUM = [32]
ROUND = 100 # 要計算幾次
...
print("ExactExplainer", shap_values)
shap.plots.waterfall(shap_values[0])
'''
explainer = shap.TreeExplainer(model.model, X_train)
shap_values = explainer(X_predictData)
print("TreeExplainer = ", shap_values)
print("shap_values = ", shap_values[0].values)
shap.plots.waterfall(shap_values[0])
...
print("midPredict = ", model.predict(midData))
print("targetPredict = ", model.predict(X_predictData))
