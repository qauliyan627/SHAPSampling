...
DATASET = 2 # 選擇資料集
ID = [186, 519, 563]
EXPLAIN_DATA = 3 # 選擇要解釋第幾筆資料(單筆解釋)
MODE = 4 # 隨機方法0, 傳統費氏(凹型)1, 黃金抽樣2, 平均費氏3, 對稱費氏(凸型)4, 分層費氏5
COMP_MODE = 4
SAMPLING_NUM = 35 # 隨機選取特徵子集的數量34, 36(mode4)
ROUND = 100 # 要計算幾次
GOLDEN_RATIO = 0.61803398875
LOCATION = f"SHAPSampling\\plot_data\\{ID[DATASET]}"
if not os.path.exists(f"{LOCATION}\\GAP\\gap_mode{COMP_MODE}.npy"): np.save(f"{LOCATION}\\GAP\\gap_mode{COMP_MODE}.npy", {-1:0})
...
if not isinstance(GAP_LIMIT, dict): GAP_LIMIT = dict()
...
if not os.path.exists(f"{LOCATION}\\ANS\\ans_{EXPLAIN_DATA}.txt"): np.savetxt(f"{LOCATION}\\ANS\\ans_{EXPLAIN_DATA}.txt", [0]*featureNum)
ANS_LIST = np.loadtxt(f"{LOCATION}\\ANS\\ans_{EXPLAIN_DATA}.txt") # 全包含的SHAP值(精準SHAP值)
...
if not MODE == 1 and not MODE == 4 and not SAMPLING_NUM == "max":
    print(f"此為ID{ID[DATASET]}資料集, 解釋第{EXPLAIN_DATA}筆資料, mode{MODE}, 總做了{ROUND}次")
